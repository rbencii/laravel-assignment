<x-guest-layout>
    ...
    <div class="container mx-auto p-3 lg:px-36 w-full">
        <form action="{{ route('items.update', $item) }}" method="POST" x-data="{ itemName: '{{ old('name', $item->name) }}', itemDesc: '{{ old('description', $item->description) }}', itemImage: '{{ old('image', $item->image) }}', obtainedDate: '{{ old('obtained', $item->obtained) }}', colorCode: '{{ old('color', $item->color) }}' }" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 place-items-center mx-auto p-3 lg:px-36 w-full">
                ...
                <div class="grid grid-cols-1 w-full">
                        ...
                        <div class="border-2 border-gray-200 p-2 rounded-lg">
                            <div class="flex flex-wrap space-x-2">
                                @foreach ($labels as $label)
                                        ...
                                        value="{{ $label->id }}" @checked(is_array(old('labels'))?in_array($label->id, old('labels')):$itemlabels->find($label->id))>
                                        </span>
                        ...
                        {{-- @foreach ($labels as $label)
                            <div class="flex flex-row pb-1">
                                <input type="checkbox" class="mt-1 mx-1" name="labels[]"
                                    value="{{ $label->id }}" {{$itemlabels->find($label->id)?"checked":""}}>
                    ...
                    <button type="submit"
                        class="mt-6 bg-blue-500 hover:bg-blue-600 text-gray-100 font-semibold px-2 py-1 text-xl">Szerkesztés</button>
